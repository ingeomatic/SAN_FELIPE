...
const SUPABASE_CONFIG = {
    url: 'https://ydugmbferktharqtoijg.supabase.co',
    key: 'eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6InlkdWdtYmZlcmt0aGFycXRvaWpnIiwicm9sZSI6ImFub24iLCJpYXQiOjE3ODE0NzY0NDIsImV4cCI6MjA5NzA1MjQ0Mn0.G5Sa1NhX27KDwgOBe3lmtbgFby1CjAmEk8zXEUWp_mc'
};
...
/* =====================================================
   ...
   ===================================================== */
const APP_CONFIG = {
    nombre_empresa:   'SISTEMA INMOBILIARIO DE LOTES',
...
const USUARIOS_COTIZADORES = {
    'judith.quispe': { password: '7309830', tipo: 'cotizador' },
    'pastor.flores': { password: '7335007', tipo: 'cotizador' },
    'viqui.machaca': { password: '7121550', tipo: 'cotizador' },
    'gigiana.alvarez': { password: '6406380', tipo: 'cotizador' },
    'franz.ramos': { password: '4075538', tipo: 'cotizador' },
    'camila.gonzales': { password: '7317889', tipo: 'cotizador' },
    'michel.zambrana': { password: '7382550', tipo: 'cotizador' },
    'estefany.tintaya': { password: '7322178', tipo: 'cotizador' },
    'dusia.olivera': { password: '5733309', tipo: 'cotizador' },
    'ahmed.fuertes': { password: '7331774', tipo: 'cotizador' }
};
...
let currentUserType = null; // 'administrador', 'secundario' o 'cotizador'

// Urbanizaciones asignadas al usuario actual (null = sin restricción = administrador)
let urbAsignadasUsuario = null;
...
function inicializarCamposFactura() {
    const mapaCampos = {
        'fact-nit-empresa':    APP_CONFIG.nit_emp,
        ...
        'res-fact-nit-empresa':  APP_CONFIG.nit_emp,
...
function checkStoredSession() {
    ...
    if (storedUser && storedUserType) {
        ...
        // Restaurar urbanizaciones asignadas desde localStorage (sesión previa)
        if (storedUserType === 'administrador') {
            urbAsignadasUsuario = null;
        } else {
            const storedUrbs = localStorage.getItem('geoportal_urb_asignadas');
            if (storedUrbs) {
                try {
                    urbAsignadasUsuario = JSON.parse(storedUrbs);
                } catch (e) {
                    urbAsignadasUsuario = null;
                }
            }
            // Refrescar desde Supabase en background para que siempre esté actualizado
            cargarUrbAsignadasDeSupabase(storedUser).then(urbs => {
                if (urbs !== null) {
                    urbAsignadasUsuario = urbs;
                    localStorage.setItem('geoportal_urb_asignadas', JSON.stringify(urbs));
                }
            });
        }
...
// loginUser es ahora async para consultar urbanizaciones asignadas en Supabase
async function loginUser(username, password) {
    let tipo = null;

    if (USUARIOS_AUTORIZADOS[username] && USUARIOS_AUTORIZADOS[username].password === password) {
        tipo = 'administrador';
    } else if (USUARIOS_SECUNDARIOS[username] && USUARIOS_SECUNDARIOS[username].password === password) {
        tipo = 'secundario';
    } else if (USUARIOS_COTIZADORES[username] && USUARIOS_COTIZADORES[username].password === password) {
        tipo = 'cotizador';
    }
    ...
    localStorage.setItem('geoportal_user_type', tipo);

    // Los administradores ven todo (sin restricción)
    if (tipo === 'administrador') {
        urbAsignadasUsuario = null;
        localStorage.removeItem('geoportal_urb_asignadas');
    } else {
        // Consultar urbanizaciones asignadas en Supabase
        urbAsignadasUsuario = await cargarUrbAsignadasDeSupabase(username);
        // Guardar en localStorage para restaurar sesión
        localStorage.setItem('geoportal_urb_asignadas', JSON.stringify(urbAsignadasUsuario));
    }
...
// Consulta Supabase para obtener las urbanizaciones asignadas al usuario
async function cargarUrbAsignadasDeSupabase(username) {
    try {
        const { data, error } = await supabaseClient
            .from('usuario_urbanizaciones')
            .select('urb_id')
            .eq('username', username);

        if (error) {
            console.warn('⚠️ No se pudieron cargar urbanizaciones asignadas:', error.message);
            return null; // Si falla, sin restricción (failsafe)
        }

        if (!data || data.length === 0) {
            console.log(`ℹ️ Usuario ${username} no tiene urbanizaciones asignadas — acceso bloqueado`);
            return []; // Array vacío = ninguna urbanización visible
        }

        const asignadas = data.map(row => String(row.urb_id));
        console.log(`✅ Urbanizaciones asignadas a ${username}:`, asignadas);
        return asignadas;
    } catch (e) {
        console.warn('⚠️ Error consultando urbanizaciones asignadas:', e);
        return null;
    }
}

// Verificar si el usuario actual puede ver una urbanización dada
function puedeVerUrbanizacion(urbId) {
    if (!isUserLoggedIn) return true; // Sin login, ver todo en modo público
    if (currentUserType === 'administrador') return true; // Admin ve todo
    if (urbAsignadasUsuario === null) return true; // Sin restricción (failsafe)
    return urbAsignadasUsuario.includes(String(urbId));
}
...
document.getElementById('user-btn').addEventListener('click', () => {
    if (isUserLoggedIn) {
        ...
        if (accessLevelText) {
            ...
            } else if (currentUserType === 'cotizador') {
                accessLevelText.textContent = 'Puedes generar cotizaciones de lotes (PDF) únicamente.';
            } else {
                accessLevelText.textContent = 'Puedes cambiar lotes DISPONIBLES a RESERVADOS únicamente.';
            }
        }
        // Mostrar/ocultar botón de gestión de urbanizaciones según rol
        const adminUrbBtnWrapper = document.getElementById('admin-urb-btn-wrapper');
        if (adminUrbBtnWrapper) {
            adminUrbBtnWrapper.style.display = currentUserType === 'administrador' ? 'block' : 'none';
        }
...
function generarPopupFotos(properties) {
    ...
    const descrip = properties.descrip || properties.DESCRIPCION || '';
    ...
    if (descrip) {
        html += `
            <div style="margin-bottom: 8px; padding: 8px 0;">
                ...
                <div style="color: #334155; font-size: 13px; font-weight: 500; margin-top: 3px;">${descrip}</div>
            </div>
        `;
    }
...
function generarPopupContent(properties, editMode = false) {
    ...
    const supm2Value = properties['sup_m2'] || 'N/A';
    const precioLot = properties.precio || 'N/A';
    ...
    if (!editMode) {
        ...
        if (isLoggedIn) {
            ...
            if (currentUserType !== 'cotizador' && (properties.estado === 'VENDIDO' || properties.estado === 'RESERVADO')) {
...
window.abrirFormularioReporte = function (featureId) {
    ...
    document.getElementById('form-Supm2').value = lote.properties['sup_m2'] || 'N/A';
...
document.getElementById('reporte-form').addEventListener('submit', async function (e) {
    ...
    const formData = {
        ...
        sup_m2: document.getElementById('form-Supm2').value,
...
async function generarPDF(formData) {
    try {
        ...
        pdf.setFont(undefined, 'normal');
        pdf.text(`${formData.sup_m2} m²`, margin + 25, yPos);
...
document.getElementById('venta-form').addEventListener('submit', async function (e) {
    ...
    const datosVenta = {
        cli_nom: document.getElementById('venta-cliente').value.trim(),
        cli_ci: document.getElementById('venta-ci').value.trim(),
        cli_cel: document.getElementById('venta-celular').value.trim(),
        prc_venta: parseFloat(document.getElementById('venta-precio').value),
    ...
    if (!datosVenta.cli_nom || !datosVenta.cli_ci || !datosVenta.cli_cel || !datosVenta.prc_venta || !datosVenta.tipo_vta || !datosVenta.vendedor) {
        showAlert('warning', 'Campos Obligatorios', 'Por favor complete todos los campos obligatorios.');
        return;
    }

    if (isNaN(datosVenta.prc_venta) || datosVenta.prc_venta <= 0) {
    ...
    const confirmar = await showConfirm(
        ...
        `¿Está seguro de ${accion} este lote para <strong>${datosVenta.cli_nom}</strong>?`,
...
async function guardarEstadoConDatos(featureId, nuevoEstado, datosVenta, urb) {
    ...
    try {
        ...
        if (nuevoEstado === 'RESERVADO') {
            ...
            updateData.cli_nom = datosVenta.cli_nom || null;
            updateData.cli_cel = datosVenta.cli_cel || null;
            updateData.vendedor = datosVenta.vendedor || currentUsername || null;
            // Limpiar datos de venta previos
            updateData.prc_venta = null;
            updateData.monto_bs = null;
            updateData.tipo_vta = null;
        } else if (nuevoEstado === 'VENDIDO') {
            // DISPONIBLE/RESERVADO → VENDIDO
            updateData.cli_nom = datosVenta.cli_nom || null;
            updateData.cli_cel = datosVenta.cli_cel || null;
            updateData.prc_venta = datosVenta.prc_venta || null;
            updateData.monto_bs = datosVenta.monto_bs || null;
            updateData.tipo_vta = datosVenta.tipo_vta || null;
            updateData.vendedor = datosVenta.vendedor || currentUsername || null;
        }

        // Intentar envío principal (con campos extendidos si existen)
        let updateDataExtended = Object.assign({}, updateData);
        if (nuevoEstado === 'RESERVADO' && datosVenta.cli_ci) updateDataExtended.cli_ci = datosVenta.cli_ci;
        if (nuevoEstado === 'RESERVADO' && datosVenta.mto_res) updateDataExtended.mto_res = datosVenta.mto_res;
        if (nuevoEstado === 'VENDIDO' && datosVenta.cli_ci) updateDataExtended.cli_ci = datosVenta.cli_ci;
...
async function guardarEstadoSinDatos(featureId, nuevoEstado, urb) {
    ...
    try {
        ...
        // ── Eliminar cobros y facturas asociados al lote ──────────
        try {
            await supabaseClient.from('cobros').delete().eq('lote_gid', featureId);
        } catch (eCobros) {
            console.warn('⚠️ No se pudieron eliminar cobros:', eCobros.message);
        }
        try {
            await supabaseClient.from('facturas').delete().eq('lote_gid', featureId);
        } catch (eFacturas) {
            console.warn('⚠️ No se pudieron eliminar facturas:', eFacturas.message);
        }
        ...
        showAlert('success', '¡Lote Disponible!', 'El lote fue cambiado a <strong>DISPONIBLE</strong> exitosamente. Los datos del cliente, cobros y facturas fueron eliminados.');
        if (currentPopup) setTimeout(() => currentPopup.remove(), 1000);
    } catch (error) {
        console.error('❌ Error al guardar:', error);
...
async function exportarExcel(urb) {
    try {
        ...
        const camposSelect = 'lote,manzano,sup_m2,precio,estado,vendedor,cli_nom,cli_cel,cli_ci,prc_venta,monto_bs,tipo_vta';
        ...
        const excelData = allData.map(row => ({
            ...
            'SUPERFICIE':            v(row.sup_m2),
            'PRECIO ($us)':          v(row.precio),
            'ESTADO':                v(row.estado),
            'VENDEDOR':              v(row.vendedor),
            'NOMBRE CLIENTE':        v(row.cli_nom),
            'CELULAR CLIENTE':       v(row.cli_cel),
            'CARNET CLIENTE':        v(row.cli_ci),
            'PRECIO DE VENTA ($us)': v(row.prc_venta),
            'PRECIO DE VENTA (Bs)':  v(row.monto_bs),
            'TIPO DE VENTA':         v(row.tipo_vta)
        }));
...
window.toggleClienteInfo = function (featureId) {
    ...
    if (esVendido) {
        const precioVentaBs = loteProps.monto_bs
            ...
            : '—';
        const precioVentaUsd = loteProps.prc_venta
            ? `$us ${parseFloat(loteProps.prc_venta).toLocaleString('es-BO', {minimumFractionDigits:2})}`
            : '—';
        const tipoBg  = loteProps.tipo_vta === 'CONTADO' ? '#dcfce7' : '#dbeafe';
        const tipoCol = loteProps.tipo_vta === 'CONTADO' ? '#166534' : '#1e40af';
        bodyHtml = `
            <div class="cfm-row"><span class="cfm-label"><i class="fas fa-user" style="color:#7c3aed;"></i>Cliente</span><span class="cfm-value">${v(loteProps.cli_nom)}</span></div>
            <div class="cfm-row"><span class="cfm-label"><i class="fas fa-id-card" style="color:#7c3aed;"></i>CI / Carnet</span><span class="cfm-value">${v(loteProps.cli_ci)}</span></div>
            <div class="cfm-row"><span class="cfm-label"><i class="fas fa-phone" style="color:#7c3aed;"></i>Celular</span><span class="cfm-value">${v(loteProps.cli_cel)}</span></div>
            ...
            <div class="cfm-row"><span class="cfm-label"><i class="fas fa-handshake" style="color:#7c3aed;"></i>Tipo de Venta</span><span class="cfm-value"><span style="background:${tipoBg};color:${tipoCol};padding:2px 10px;border-radius:20px;font-weight:700;font-size:12px;">${v(loteProps.tipo_vta)}</span></span></div>
        `;
    } else {
        const montoRes = loteProps.mto_res
            ? `Bs ${parseFloat(loteProps.mto_res).toLocaleString('es-BO', {minimumFractionDigits:2})}`
            : '—';
        bodyHtml = `
            <div class="cfm-row"><span class="cfm-label"><i class="fas fa-user" style="color:#d97706;"></i>Cliente</span><span class="cfm-value">${v(loteProps.cli_nom)}</span></div>
            <div class="cfm-row"><span class="cfm-label"><i class="fas fa-id-card" style="color:#d97706;"></i>CI / Carnet</span><span class="cfm-value">${v(loteProps.cli_ci)}</span></div>
            <div class="cfm-row"><span class="cfm-label"><i class="fas fa-phone" style="color:#d97706;"></i>Celular</span><span class="cfm-value">${v(loteProps.cli_cel)}</span></div>
            <div class="cfm-divider"></div>
            <div class="cfm-row"><span class="cfm-label"><i class="fas fa-user-tie" style="color:#1e40af;"></i>Vendedor</span><span class="cfm-value">${v(loteProps.vendedor)}</span></div>
...
window.guardarEstado = async function (featureId) {
    ...
    // Validación para usuarios cotizadores (solo cotizaciones, sin editar estados)
    if (currentUserType === 'cotizador') {
        showAlert('warning', 'Permiso Denegado', 'Como usuario cotizador, no tienes permiso para cambiar el estado de los lotes.');
    ...
    if (nuevoEstado === 'VENDIDO' && estadoActual === 'RESERVADO') {
        ...
        document.getElementById('venta-cliente').value = props.cli_nom || '';
        document.getElementById('venta-ci').value = props.cli_ci || '';
        document.getElementById('venta-celular').value = props.cli_cel || '';
        document.getElementById('venta-vendedor').value = currentUsername || '';

        // Mostrar monto de reserva si existe
        if (props.mto_res && parseFloat(props.mto_res) > 0) {
            document.getElementById('venta-monto-reserva-info').style.display = 'block';
            document.getElementById('venta-reserva-monto-display').textContent = `Bs ${parseFloat(props.mto_res).toLocaleString('es-BO', { minimumFractionDigits: 2 })}`;
        } else {
            document.getElementById('venta-monto-reserva-info').style.display = 'none';
        }
...
function inicializarCamposEmpresa() {
    // Factura de cobro
    const campos = [
        ['fact-nit-empresa',    'res-fact-nit-empresa',    APP_CONFIG.nit_emp],
...
map.on('load', async () => {
    try {
        ...
        // Filtrar urbanizaciones según asignaciones del usuario
        const urbVisibles = Object.keys(urbData).filter(urb => puedeVerUrbanizacion(urb));

        if (urbVisibles.length === 0 && currentUserType !== 'administrador' && isUserLoggedIn) {
            urbLayersDiv.innerHTML = '<div style="padding:12px;color:#b45309;font-size:13px;text-align:center;"><i class="fas fa-lock" style="margin-right:6px;"></i>Sin urbanizaciones asignadas.<br>Contacta al administrador.</div>';
        }

        urbVisibles.forEach(urb => {
            const div = document.createElement('div');
...
document.getElementById('cobros-urb-select').addEventListener('change', async (e) => {
    ...
    try {
        ...
        const result1 = await supabaseClient
            .from('lotes')
            .select('gid, lote, manzano, estado, cli_nom, prc_venta, monto_bs, mto_res, cli_ci, cli_cel, nro_cuotas, tipo_vta')
            .eq('urb', urb)
            .eq('estado', 'VENDIDO')
            .order('manzano')
            .order('lote');
        ...
        if (result1.error && (result1.error.message.includes('column') || result1.error.code === 'PGRST204')) {
            ...
            const result2 = await supabaseClient
                .from('lotes')
                .select('gid, lote, manzano, estado, cli_nom, prc_venta, monto_bs, tipo_vta, cli_cel')
                .eq('urb', urb)
                .eq('estado', 'VENDIDO')
                .order('manzano')
                .order('lote');
    ...
    try {
        ...
        try {
            const { data: facAd } = await supabaseClient
                .from('facturas')
                .select('nro_fact, tipo_fact, total, fch_emit, nom_cli')
                .eq('lote_gid', loteGid)
                .ilike('tipo_fact', 'ADELANTO%')
                .order('fch_emit', { ascending: true })
                .limit(1)
                .maybeSingle();
        ...
        // SIEMPRE mostrar el adelanto si mto_res > 0 en el lote
        // La factura es opcional (si existe muestra el N°, sino muestra solo el monto)
        const montoReserva = parseFloat(loteSeleccionadoCobro?.mto_res || 0);
...
function mostrarHistorialCobros(montoReserva = 0, facturaAdelanto = null) {
    ...
    if (tieneAdelanto) {
        ...
        const fechaAd = facturaAdelanto?.fch_emit
            ? new Date(facturaAdelanto.fch_emit).toLocaleDateString('es-BO')
            : '—';
        const nroFac = facturaAdelanto?.nro_fact || null;
        html += `
    ...
    cobrosDelLote.forEach(cobro => {
        ...
        const fecha = new Date(cobro.fch_cobro).toLocaleDateString('es-BO');
        const tieneFactura = !!cobro.nro_fact;
            ...
            <div style="padding:14px 16px; background:#f9fafb; border:1px solid #e5e7eb; border-radius:10px; position:relative; overflow:hidden;">
                ...
                <div style="padding-left:8px;">
                    ...
                    <div style="font-size:12px; color:#6b7280; display:flex; flex-wrap:wrap; gap:8px;">
                        ...
                        ${tieneFactura ? `<span><i class="fas fa-receipt"></i> Fac. N° ${cobro.nro_fact}</span>` : ''}
    ...
    if (saldoFinal <= 0 && montoBs > 0) {
        ...
        <div style="margin-top:14px; padding:18px 16px; background:linear-gradient(135deg,#d1fae5,#a7f3d0); border:2px solid #10b981; border-radius:14px; text-align:center; box-shadow:0 4px 14px rgba(16,185,129,0.18);">
            ...
            <div style="font-size:13px; color:#047857; font-weight:600;">
                ${loteSeleccionadoCobro?.cli_nom ? '<i class="fas fa-user-check"></i> ' + loteSeleccionadoCobro.cli_nom + ' ha' : 'El cliente ha'} terminado de cancelar el lote.
            </div>
...
function mostrarInfoLote() {
    ...
    const montoReserva = parseFloat(lote.mto_res || 0);
    document.getElementById('cobros-lote-info').style.display = 'block';
    document.getElementById('info-cliente').textContent = lote.cli_nom || 'Sin registro';
    // CI
    const ciEl = document.getElementById('info-ci');
    if (ciEl) ciEl.textContent = lote.cli_ci || '—';
    // Precio en USD
    document.getElementById('info-precio').textContent = lote.prc_venta ?
        `$us ${parseFloat(lote.prc_venta).toLocaleString('es-BO', { minimumFractionDigits: 2 })}` :
        'No registrado';
...
function renderizarListaLotes(lotes) {
    ...
    sorted.forEach(lote => {
        ...
        option.textContent = `${manzanoText}Lt ${lote.lote} — ${lote.cli_nom || 'Sin cliente'}`;
...
document.getElementById('cobros-lote-buscar').addEventListener('input', (e) => {
    ...
    const lotesFiltrados = lotesDisponiblesParaCobro.filter(lote => {
        ...
        const cliente = (lote.cli_nom || '').toLowerCase();
...
document.getElementById('registrar-cobro-btn').addEventListener('click', () => {
    ...
    document.getElementById('cobro-resumen-nombre').textContent = lote.cli_nom || 'Sin registro';
    document.getElementById('cobro-resumen-ci').textContent = lote.cli_ci || '—';
    document.getElementById('cobro-resumen-cel').textContent = lote.cli_cel || '—';
    ...
    document.getElementById('fact-cliente-ci').value = lote.cli_ci || '';
    document.getElementById('fact-cliente-nombre').value = lote.cli_nom || '';

    // Descripción automática
    const mzText = lote.manzano ? `Mz ${lote.manzano} ` : '';
    document.getElementById('fact-descripcion').value =
        `Pago Cuota N°${proximaCuota} — Lote ${lote.lote || '?'} ${mzText}— ${lote.cli_nom || ''}`;
...
document.getElementById('omitir-factura-cobro-btn').addEventListener('click', async () => {
    ...
    // Aplicar descuento por adelanto (cuota 1) al monto neto guardado en cobros
    const descuentoSF = parseFloat(document.getElementById('fact-descuento').value) || 0;
    const montoNetoSF = Math.max(0, monto - descuentoSF);

    const confirmar = await showConfirm(
        'Guardar SIN Factura',
        `¿Desea registrar el cobro de <strong>Bs ${montoNetoSF.toFixed(2)}</strong> <strong>sin generar factura</strong>?<br><br>El cobro quedará registrado con indicación <em>"SIN FACTURA"</em>. <strong>No podrá generarse factura después.</strong>`,
    ...
    try {
        ...
        const cobro_payload_sf = {
            ...
            monto: montoNetoSF,
            fch_cobro: new Date(fecha + 'T12:00:00').toISOString(),
            nro_fact: 'SIN FACTURA',
            nro_cuota: cuota,
...
document.getElementById('nuevo-cobro-form').addEventListener('submit', async (e) => {
    ...
    // Aplicar descuento por adelanto (cuota 1): el monto neto es lo que realmente se guarda en cobros
    const descuentoForm = parseFloat(document.getElementById('fact-descuento').value) || 0;
    const montoNeto = Math.max(0, monto - descuentoForm);

    // Validación: no permitir cobro mayor al saldo
    // Saldo = precio total - adelanto ya pagado - cuotas ya cobradas (netas)
    const montoBs = parseFloat(loteSeleccionadoCobro.monto_bs || 0);
    const totalCuotasCobradas = cobrosDelLote.reduce((sum, c) => sum + parseFloat(c.monto), 0);
    const adelantoPagado = parseFloat(loteSeleccionadoCobro.mto_res || 0);
    const saldo = montoBs - adelantoPagado - totalCuotasCobradas;

    if (montoNeto > saldo && saldo > 0) {
        const continuar = await showConfirm(
            '¿Monto excede el saldo?',
            `El monto a registrar (<strong>Bs ${montoNeto.toFixed(2)}</strong>) es mayor al saldo pendiente (<strong>Bs ${saldo.toFixed(2)}</strong>).<br><br>¿Desea continuar de todas formas?`,
    ...
    const confirmar = await showConfirm(
        'Confirmar Cobro',
        `¿Está seguro de registrar el cobro por <strong>Bs ${montoNeto.toFixed(2)}</strong>?`,
    ...
    try {
        ...
        showAlert('success', '¡Cobro Registrado!', `El cobro de <strong>Bs ${montoNeto.toFixed(2)}</strong> fue registrado exitosamente.`);
        document.getElementById('nuevo-cobro-modal').style.display = 'none';
        document.getElementById('nuevo-cobro-form').reset();
        await cargarHistorialCobros(loteSeleccionadoCobro.gid);
    } catch (error) {
...
document.getElementById('generar-factura-cobro-btn').addEventListener('click', async () => {
    ...
    // Aplicar descuento por adelanto (cuota 1): el monto neto es lo que se guarda en cobros
    const descuentoFac = parseFloat(document.getElementById('fact-descuento').value) || 0;
    const montoNeto2 = Math.max(0, monto - descuentoFac);
    ...
    try {
        // 1. Guardar cobro en Supabase con monto neto (descontado el adelanto si es cuota 1)
        const urb = document.getElementById('cobros-urb-select').value;
...
function abrirFacturaConDatos(lote, monto, cuota, fecha) {
    document.getElementById('fact-cliente-ci').value = lote.cli_ci || lote.cli_nom || '';
    document.getElementById('fact-cliente-nombre').value = lote.cli_nom || '';
    const mzText = lote.manzano ? `Mz ${lote.manzano} ` : '';
    document.getElementById('fact-descripcion').value =
        `Pago Cuota N°${cuota} — Lote ${lote.lote} ${mzText}— ${lote.cli_nom || ''}`;
...
async function generarPDFFactura(guardar = true) {
    ...
    const descrip = document.getElementById('fact-descripcion').value.trim();
    const cantidad = parseFloat(document.getElementById('fact-cantidad').value) || 1;
    ...
    try {
        ...
        const descLines = pdf.splitTextToSize(descrip, COL_W.desc - 4);
        ...
        if (guardar) {
            try {
                await supabaseClient.from('facturas').insert([{
                    nro_fact: nroFactura,
                    tipo_fact: tipoFactura,
                    cuf: cuf,
                    nit_emp: nitEmpresa,
                    rzn_social: razonSocial,
                    nit_ci: clienteCI,
                    nom_cli: clienteNombre,
                    descrip: descrip,
                    cantidad: cantidad,
                    prc_unit: montoUnit,
                    descuento: descuento,
                    total: total,
                    fch_emit: new Date().toISOString(),
                    usr_emisor: currentUsername,
...
document.getElementById('reserva-form').addEventListener('submit', async (e) => {
    ...
    const datosReserva = {
        cli_nom: document.getElementById('reserva-cliente').value.trim(),
        cli_ci: document.getElementById('reserva-ci').value.trim(),
        cli_cel: document.getElementById('reserva-celular').value.trim(),
        mto_res: parseFloat(document.getElementById('reserva-monto').value) || 0,
        vendedor: currentUsername
    };

    if (!datosReserva.cli_nom || !datosReserva.cli_ci || !datosReserva.cli_cel || datosReserva.mto_res <= 0) {
    ...
    const confirmar = await showConfirm(
        'Confirmar Reserva',
        `¿Está seguro de registrar la reserva para <strong>${datosReserva.cli_nom}</strong> con un monto de <strong>Bs ${datosReserva.mto_res.toLocaleString('es-BO', { minimumFractionDigits: 2 })}</strong>?`,
...
function abrirFacturaDesdeReserva(gid, loteProps, datosReserva) {
    const montoReserva = datosReserva.mto_res || 0;
    ...
    document.getElementById('res-fac-nombre').textContent = datosReserva.cli_nom || '—';
    document.getElementById('res-fac-ci').textContent = datosReserva.cli_ci || '—';
    document.getElementById('res-fac-cel').textContent = datosReserva.cli_cel || '—';
    ...
    document.getElementById('res-fact-cliente-ci').value = datosReserva.cli_ci || '';
    document.getElementById('res-fact-cliente-nombre').value = datosReserva.cli_nom || '';
    document.getElementById('res-fact-monto').value = montoReserva.toFixed(2);
    document.getElementById('res-fact-descuento').value = '0';
    document.getElementById('res-fact-descripcion').value = `Adelanto/Reserva — ${loteDesc} — ${datosReserva.cli_nom || ''}`;
    ...
    loteReservadoSeleccionado = {
        gid,
        cli_nom: datosReserva.cli_nom,
        cli_ci: datosReserva.cli_ci,
        cli_cel: datosReserva.cli_cel,
        mto_res: montoReserva,
        lote: loteProps.lote,
...
async function cargarDashboard() {
    ...
    try {
        ...
        while (true) {
            const { data, error } = await supabaseClient
                .from('lotes')
                .select('urb, estado, monto_bs, tipo_vta, mto_res', { count: 'exact' })
        ...
        const totalReservaDeVendidos = allLotes
            .filter(l => l.estado === 'VENDIDO' && l.mto_res && parseFloat(l.mto_res) > 0)
            .reduce((s, l) => s + parseFloat(l.mto_res || 0), 0);

        // ── Adelantos de lotes RESERVADOS (dinero ingresado, lote aún reservado) ──
        const totalAdelantosReservados = allLotes
            .filter(l => l.estado === 'RESERVADO' && l.mto_res && parseFloat(l.mto_res) > 0)
            .reduce((s, l) => s + parseFloat(l.mto_res || 0), 0);
        ...
        allLotes.filter(l => l.mto_res && parseFloat(l.mto_res) > 0).forEach(l => {
            const n = URB_NOMBRES[l.urb] || `URB ${l.urb}`;
            cobradoPorUrb[n] = (cobradoPorUrb[n] || 0) + parseFloat(l.mto_res || 0);
        });
            ...
            <div style="display:grid; grid-template-columns:1fr 2px 1fr 2px 1fr; gap:0; align-items:stretch;">
                ...
                <div style="padding:10px 18px;">
                    ...
                    <div style="font-size:12px; color:#374151; margin-bottom:3px;">
                        ...
                        Lotes con adelanto: <strong style="color:#d97706;">${allLotes.filter(l=>l.estado==='RESERVADO'&&l.mto_res&&parseFloat(l.mto_res)>0).length} lotes</strong>
                    </div>
...
async function mostrarReporteVentas(contenido, desde, hasta, urb, vendedorFiltro) {
    let query = supabaseClient.from('lotes').select('gid, vendedor, cli_nom, cli_ci, cli_cel, lote, manzano, prc_venta, monto_bs, tipo_vta, estado, urb, nro_cuotas, mto_res')
        .eq('estado', 'VENDIDO');
    ...
    if (gids.length > 0) {
        const { data: cv } = await supabaseClient.from('cobros')
            .select('lote_gid, monto, nro_fact, nro_cuota, fch_cobro, usuario')
            .in('lote_gid', gids);
        cobrosVentas = cv || [];
    }
    ...
    cobrosVentas.forEach(c => {
        ...
        if (c.nro_fact) {
            ...
            facturasPorLote[c.lote_gid].push(c.nro_fact);
        }
    });
    // ── INCLUIR mto_res en el total cobrado por lote (adelanto que pasó a venta) ──
    (data || []).forEach(l => {
        if (l.mto_res && parseFloat(l.mto_res) > 0) {
            cobradoPorLote[l.gid] = (cobradoPorLote[l.gid] || 0) + parseFloat(l.mto_res);
        }
    });
    ...
    if (!vendedorFiltro) {
        ...
        ranking.forEach(([vendedor, d], i) => {
            const contado = d.lotes.filter(l => l.tipo_vta === 'CONTADO').length;
            const credito = d.lotes.filter(l => l.tipo_vta === 'CREDITO').length;
        ...
        (data || []).forEach(l => {
            ...
            html += `<tr>
                ...
                <td>${l.cli_nom || '-'}</td><td>${l.cli_ci || '-'}</td>
                <td>$us ${parseFloat(l.prc_venta || 0).toLocaleString('es-BO', { minimumFractionDigits: 2 })}</td>
                ...
                <td><span style="background:${l.tipo_vta === 'CONTADO' ? '#dcfce7' : '#dbeafe'}; padding:2px 8px; border-radius:10px; font-size:11px; font-weight:600;">${l.tipo_vta || '-'}</span></td>
        ...
        dv.lotes.forEach(l => {
            ...
            html += `<tr>
                <td>${URB_NOMBRES[l.urb] || l.urb}</td>
                <td>${l.manzano || '-'}</td><td>${l.lote || '-'}</td>
                <td><strong>${l.cli_nom || '-'}</strong></td>
                <td>${l.cli_ci || '-'}</td><td>${l.cli_cel || '-'}</td>
                <td>$us ${parseFloat(l.prc_venta || 0).toLocaleString('es-BO', { minimumFractionDigits: 2 })}</td>
                <td>Bs ${parseFloat(l.monto_bs || 0).toLocaleString('es-BO', { minimumFractionDigits: 2 })}</td>
                <td style="color:#059669; font-weight:600;">Bs ${cob.toLocaleString('es-BO',{maximumFractionDigits:0})}</td>
...
async function generarPDFVendedorIndividual() {
    ...
    if (vendedor !== 'TODOS') {
        ...
        pdf.text(`Contado: ${dv.lotes.filter(l=>l.tipo_vta==='CONTADO').length}  |  Crédito: ${dv.lotes.filter(l=>l.tipo_vta==='CREDITO').length}`, 20, y+13);
        y += 22;
    }
    ...
    lotes.forEach((l, ri) => {
        ...
        const cells = [
            ...
            l.manzano||'-', l.lote||'-',
            (l.cli_nom||'-').substring(0,20),
            l.cli_ci||'-',
            '$us '+parseFloat(l.prc_venta||0).toFixed(0),
...
async function mostrarReporteClientes(contenido, desde, hasta, urb) {
    let query = supabaseClient.from('lotes').select('gid, cli_nom, cli_cel, cli_ci, lote, manzano, prc_venta, monto_bs, tipo_vta, estado, urb, mto_res, vendedor')
    ...
    // Cobros por lote
    const { data: cobros } = await supabaseClient.from('cobros').select('lote_gid, monto, fch_cobro, nro_cuota');
    const cobradoPorLote = {};
    ...
    (lotes || []).forEach(l => {
        ...
        const adelanto = parseFloat(l.mto_res || 0);
        ...
        const tipoBg = l.tipo_vta === 'CONTADO' ? '#dcfce7' : '#dbeafe';
        const tipoTxt = l.tipo_vta === 'CONTADO' ? '#166534' : '#1e40af';
        html += `<tr>
            <td><strong>${l.cli_nom || '-'}</strong></td>
            <td>${l.cli_ci || '-'}</td>
            <td>${l.cli_cel || '-'}</td>
            ...
            <td><span style="background:${tipoBg}; color:${tipoTxt}; padding:2px 8px; border-radius:10px; font-size:11px; font-weight:600;">${l.tipo_vta || '-'}</span></td>
...
async function mostrarReportePagosCliente(contenido, desde, hasta, urb, clienteFiltroKey) {
    ...
    let query = supabaseClient.from('lotes')
        .select('gid, cli_nom, cli_ci, cli_cel, lote, manzano, prc_venta, monto_bs, tipo_vta, estado, urb, mto_res, vendedor, nro_cuotas')
        .in('estado', ['VENDIDO', 'RESERVADO'])
        .not('cli_nom', 'is', null);
    if (urb) query = query.eq('urb', urb);
    ...
    const { data: cobros } = await supabaseClient.from('cobros')
        .select('lote_gid, monto, fch_cobro, nro_cuota, nro_fact, usuario');
    const cobrosPorLote = {};
    (cobros || []).forEach(c => {
    ...
    const clientesMap = {};
    (lotes || []).forEach(l => {
        const key = (l.cli_ci || l.cli_nom || 'SIN_CI').toUpperCase().trim();
        if (!clientesMap[key]) clientesMap[key] = { nombre: l.cli_nom, ci: l.cli_ci, cel: l.cli_cel, lotes: [] };
    ...
    clientesAMostrar.forEach(([key, cliente]) => {
        ...
        cliente.lotes.forEach(l => {
            const cobrosList = (cobrosPorLote[l.gid] || []).sort((a, b) => new Date(a.fch_cobro) - new Date(b.fch_cobro));
            const montoBs = parseFloat(l.monto_bs || 0);
            const adelanto = parseFloat(l.mto_res || 0);
            ...
            html += `<div style="padding:14px 16px; background:#fafbff; border-bottom:1px solid #f1f5f9;">
                <div style="display:flex; gap:16px; flex-wrap:wrap; margin-bottom:10px; padding:10px 14px; background:#eff6ff; border-radius:8px; border-left:4px solid #3b82f6; font-size:13px;">
                    ...
                    <span><strong>Tipo:</strong> <span style="color:${l.tipo_vta==='CONTADO'?'#059669':'#7c3aed'};">${l.tipo_vta || '-'}</span></span>
            ...
            if (cobrosList.length > 0) {
                ...
                cobrosList.forEach(c => {
                    const fecha = c.fch_cobro ? new Date(c.fch_cobro).toLocaleDateString('es-BO') : '-';
                    html += `<tr>
                        ...
                        <td style="font-size:11px; color:#64748b;">${c.nro_fact || '—'}</td>
...
async function generarPDFPagosCliente() {
    ...
    for (const l of cliente.lotes) {
        ...
        const montoBs = parseFloat(l.monto_bs || 0);
        const adelanto = parseFloat(l.mto_res || 0);
        const cobrosList = (datos.cobrosPorLote[l.gid] || []).sort((a,b) => new Date(a.fch_cobro)-new Date(b.fch_cobro));
        const totalCuotas = cobrosList.reduce((s,c) => s+parseFloat(c.monto||0), 0);
        const totalIngresado = totalCuotas + adelanto;
        const saldo = Math.max(0, montoBs - totalIngresado);
        ...
        pdf.text(`Estado: ${l.estado}  |  Tipo: ${l.tipo_vta||'-'}  |  Precio Total: Bs ${montoBs.toLocaleString('es-BO',{maximumFractionDigits:0})}`, mg+3, y+25);
...
async function generarPDFFacturaReserva() {
    ...
    const descrip = document.getElementById('res-fact-descripcion').value.trim();
    ...
    try {
        ...
        // Fila dinámica
        const resDescLines = pdf.splitTextToSize(descrip, RES_COL_W.desc - 4);
        ...
        try {
            await supabaseClient.from('facturas').insert([{
                nro_fact: nroFactura,
                tipo_fact: 'ADELANTO-' + tipoFactura,
                cuf: cuf,
                nit_emp: nitEmpresa,
                rzn_social: razonSocial,
                nit_ci: clienteCI,
...
/* =====================================================
   MÓDULO: GESTIÓN DE URBANIZACIONES POR USUARIO (Solo Administrador)
   ===================================================== */

// Abrir modal de gestión (llamado desde el botón en el modal de usuario)
async function abrirAdminUrbanizaciones() {
    if (!isUserLoggedIn || currentUserType !== 'administrador') {
        showAlert('warning', 'Acceso Denegado', 'Solo los administradores pueden gestionar asignaciones de urbanizaciones.');
        return;
    }
    const modal = document.getElementById('admin-urb-modal');
    if (!modal) return;
    modal.classList.add('active');
    await cargarTablaAdminUrb();
}
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    // Modal de gestión de urbanizaciones
    const adminUrbModal = document.getElementById('admin-urb-modal');
    if (adminUrbModal) {
        const closeBtn = document.getElementById('close-admin-urb-modal');
        if (closeBtn) {
            closeBtn.addEventListener('click', () => adminUrbModal.classList.remove('active'));
        }
        adminUrbModal.addEventListener('click', (e) => {
            if (e.target === adminUrbModal) adminUrbModal.classList.remove('active');
        });
    }

    // Botón de gestión en el modal de usuario
    const adminUrbBtn = document.getElementById('admin-urb-btn');
    if (adminUrbBtn) {
        adminUrbBtn.addEventListener('click', () => {
            // Cerrar el modal de usuario primero
            document.getElementById('user-modal').classList.remove('active');
            abrirAdminUrbanizaciones();
        });
    }
});

// Todos los usuarios no-admin
function obtenerTodosLosUsuarios() {
    const todos = [];
    Object.entries(USUARIOS_SECUNDARIOS).forEach(([u, d]) => todos.push({ username: u, tipo: 'secundario', label: 'Secundario' }));
    Object.entries(USUARIOS_COTIZADORES).forEach(([u, d]) => todos.push({ username: u, tipo: 'cotizador', label: 'Cotizador' }));
    return todos;
}

async function cargarTablaAdminUrb() {
    const contenido = document.getElementById('admin-urb-contenido');
    if (!contenido) return;
    contenido.innerHTML = '<div style="text-align:center;padding:30px;color:#64748b;"><i class="fas fa-spinner fa-spin" style="font-size:24px;"></i><br><br>Cargando datos...</div>';

    try {
        // Obtener todas las asignaciones actuales
        const { data: asignaciones, error } = await supabaseClient
            .from('usuario_urbanizaciones')
            .select('username, urb_id');

        if (error) throw error;

        // Construir mapa de asignaciones: { username: Set<urb_id> }
        const mapaAsig = {};
        (asignaciones || []).forEach(row => {
            if (!mapaAsig[row.username]) mapaAsig[row.username] = new Set();
            mapaAsig[row.username].add(String(row.urb_id));
        });

        const usuarios = obtenerTodosLosUsuarios();
        const urbanizaciones = Object.entries(URB_NOMBRES); // [['1', 'SAAVEDRA'], ...]

        if (usuarios.length === 0) {
            contenido.innerHTML = '<p style="color:#64748b;text-align:center;padding:20px;">No hay usuarios secundarios ni cotizadores registrados.</p>';
            return;
        }
        ...
        <table style="width:100%;border-collapse:collapse;font-size:13px;">
            <thead>
                <tr style="background:linear-gradient(135deg,#1a5248,#217a6a);color:#fff;">
                    <th style="padding:12px 14px;text-align:left;font-weight:700;border-radius:8px 0 0 0;">Usuario</th>
                    <th style="padding:12px 14px;text-align:center;font-weight:700;">Tipo</th>
                    ${urbanizaciones.map(([id, nombre]) => `<th style="padding:12px 10px;text-align:center;font-weight:700;white-space:nowrap;">${nombre}</th>`).join('')}
                    <th style="padding:12px 14px;text-align:center;font-weight:700;border-radius:0 8px 0 0;">Guardar</th>
                </tr>
            </thead>
            <tbody>
        `;
        ...
        usuarios.forEach((u, idx) => {
            ...
            const asigSet = mapaAsig[u.username] || new Set();
            const tipoBg = u.tipo === 'cotizador' ? '#ede9fe' : '#e0f2fe';
            const tipoColor = u.tipo === 'cotizador' ? '#6d28d9' : '#0369a1';

            html += `
            <tr style="background:${bg};border-bottom:1px solid #e2e8f0;" id="row-${u.username.replace(/\./g, '_')}">
                <td style="padding:10px 14px;font-weight:600;color:#1e3a5f;">
                    <i class="fas fa-user" style="color:#64748b;margin-right:6px;"></i>${u.username}
                </td>
                <td style="padding:10px;text-align:center;">
                    <span style="background:${tipoBg};color:${tipoColor};padding:3px 10px;border-radius:20px;font-size:11px;font-weight:700;">${u.label}</span>
                </td>
                ${urbanizaciones.map(([id, nombre]) => `
                    <td style="padding:10px;text-align:center;">
                        <input type="checkbox" 
                            id="chk-${u.username.replace(/\./g,'_')}-${id}"
                            data-username="${u.username}"
                            data-urb="${id}"
                            class="urb-asig-chk"
                            ${asigSet.has(String(id)) ? 'checked' : ''}
                            style="width:18px;height:18px;cursor:pointer;accent-color:#217a6a;">
                    </td>
                `).join('')}
                <td style="padding:10px;text-align:center;">
                    <button onclick="guardarAsignacionUsuario('${u.username}')" 
                        id="btn-save-${u.username.replace(/\./g,'_')}"
                        style="background:linear-gradient(135deg,#217a6a,#2ea88f);color:#fff;border:none;padding:7px 14px;border-radius:8px;cursor:pointer;font-size:12px;font-weight:700;transition:opacity 0.2s;">
        ...
        // Info adicional
        html += `
        <div style="margin-top:16px;padding:12px 16px;background:#eff9f5;border:1px solid #a8dece;border-radius:10px;font-size:12px;color:#164035;">
            <i class="fas fa-info-circle" style="color:#217a6a;margin-right:6px;"></i>
            <strong>Nota:</strong> Si un usuario no tiene ninguna urbanización marcada, no podrá visualizar ninguna en el sistema.
            Los administradores siempre ven todas las urbanizaciones sin restricciones.
        </div>`;
        ...
        contenido.innerHTML = `<div style="padding:20px;color:#dc2626;text-align:center;"><i class="fas fa-exclamation-triangle"></i><br><br>Error al cargar datos: ${e.message}</div>`;
    }
}

window.guardarAsignacionUsuario = async function(username) {
    const safeKey = username.replace(/\./g, '_');
    const btn = document.getElementById(`btn-save-${safeKey}`);
    const originalHTML = btn ? btn.innerHTML : '';
    if (btn) { btn.disabled = true; btn.innerHTML = '<i class="fas fa-spinner fa-spin"></i>'; }

    try {
        // Recoger checkboxes marcados para este usuario
        const checkboxes = document.querySelectorAll(`.urb-asig-chk[data-username="${username}"]`);
        const urbsSeleccionadas = [];
        checkboxes.forEach(chk => {
            if (chk.checked) urbsSeleccionadas.push(chk.dataset.urb);
        });
        ...
        // 2. Insertar las nuevas asignaciones (si hay alguna)
        if (urbsSeleccionadas.length > 0) {
            const inserts = urbsSeleccionadas.map(urb_id => ({ username, urb_id }));
        ...
        showToast('success', `✅ Urbanizaciones de "${username}" actualizadas correctamente`);

        // Si el usuario actualmente en sesión es el afectado, refrescar sus asignaciones
        if (currentUsername === username) {
            urbAsignadasUsuario = urbsSeleccionadas.length > 0 ? urbsSeleccionadas : [];
            localStorage.setItem('geoportal_urb_asignadas', JSON.stringify(urbAsignadasUsuario));
        }

    } catch (e) {
        console.error('Error guardando asignaciones:', e);
        showAlert('error', 'Error al Guardar', 'No se pudieron guardar las asignaciones: ' + e.message);
    } finally {
        if (btn) { btn.disabled = false; btn.innerHTML = originalHTML || '<i class="fas fa-save"></i> Guardar'; }
    }
};
